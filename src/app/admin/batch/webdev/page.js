...
async function getData() {
  try {
    ...
    const webDevBatches = result.filter((res) => res.batchCourse === 'Web Development');
  ...
  return (
    <main className={styles.main}>
      ...
      <div className={styles.right}>
        ...
        <h1 className={styles.header}>Web Development(Full Stack) Batch Details</h1>
          ...
          <table className={styles.table}>
            ...
            {renderData.map((item, index) => (
              <tr key={index} className={styles.tr}>
                ...
                <td className={`${styles.td}`} onClick={() => handleStudentClick(item)} style={{ cursor: 'pointer' }}>{item.batchUsers ? (item.batchUsers.length === 0 ? 'null' : item.batchUsers[0]):'null'}</td>
