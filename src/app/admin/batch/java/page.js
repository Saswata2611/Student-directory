...
async function getData() {
  try {
    ...
    const webDevBatches = result.filter((res) => res.batchCourse === 'Java Development');
...
const handleDeleteStudent = async () => {
  ...
  if (student && newBatchName) {
    try {
      const response = await fetch(
        `https://main-project-for-avik-sir.onrender.com/deletestudent?batch_name=${newBatchName}&username=${student}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <main className={styles.main}>
      ...
      <div className={styles.right}>
        ...
        <h1 className={styles.header}>Java Development Batch Details</h1>
          ...
          <table className={styles.table}>
            ...
            {renderData.map((item, index) => (
              <tr key={index} className={styles.tr}>
                ...
                <td className={`${styles.td}`} onClick={() => handleStudentClick(item)} style={{ cursor: 'pointer' }}>{item.batchUsers === null ? 'null' : item.batchUsers[0]}</td>
